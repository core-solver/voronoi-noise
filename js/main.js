...
vec2 hitBox(vec3 orig, vec3 dir) {
  const vec3 box_min = vec3(-.7);
  const vec3 box_max = vec3(.65);
  ...
  float t1 = min(tmax.x, max(tmax.y, tmax.z));
  return vec2(t0, t1);
}
...
vec2 hitBox(vec3 orig, vec3 dir) {
  const vec3 box_min = vec3(-.3);
  const vec3 box_max = vec3(.3);
  vec3 inv_dir = 1.0 / dir;
  vec3 tmin_tmp = (box_min - orig) * inv_dir;
  vec3 tmax_tmp = (box_max - orig) * inv_dir;
  vec3 tmin = min(tmin_tmp, tmax_tmp);
...
float w( vec3 p ) {
  return max( sphere( p, 0.9 ), distort( p * 6.0 + time * 0.8, 4.0 ) );
}
  ...
  */
  ...
  if(accumulate) {
    ...
    for (float t = bounds.x; t < bounds.y; t += delta) {
      ...
      //vec3 col = ( p * 2.0 + 0.25 );
      vec3 col = normal(p + 5.5) * 0.5 + ( p * 2.50 + 0.25 );
